...
public class PlayerShip : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // float aY = CrossPlatformInputManager.GetAxis("Vertical");
                float aX  = joystick.Horizontal;
                float aY  = joystick.Vertical;
    ...
    {
        if (collision.gameObject.name == "hasExtraLife")
        { 
        hasExtraLife(collision.gameObject);
        return;
        }
        ExplosionSound.Play();
    ...
    void hasExtraLife(GameObject g){
        JUMPS=JUMPS+1;
        Destroy(g);
    }
